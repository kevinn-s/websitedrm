...
Lihat berbagai kegiatan yang tersedia, baik secara langsung maupun daring." />
    <div class="max-w-5xl w-full mx-auto my-6 sm:my-10 px-4 sm:px-6 lg:px-0" x-data="{
            ...
            annualType: '{{ \App\Enums\EventType::Annual->value }}',
            setActiveTab(type) {
                this.activeTab = type;
                $wire.setActiveType(this.activeTab);
                this.loading = true;
                this.startTimeOnLoading = Date.now();
            },
            loading: false,
            init() {
                Livewire.on('loading', () => {
                ...
                Livewire.on('unloading', () => {
                    const elapsed = Date.now() - this.startTimeOnLoading;
                    if (elapsed < 1000) {
        ...
        <div class="min-h-32 h-full flex justify-center items-center">
            <div role="status" x-show="loading">
                <svg aria-hidden="true" class="inline w-8 h-8 text-gray-200 animate-spin fill-primary-green-500"
            ...
            >
                ...
                @foreach ($events as $event)
                    @php
                        ...
                        if ($event->type === EventType::Scheduled && $access) {
                            switch ($access->type) {
                                case EventAccessType::VIRTUAL:
                                    $location = 'Online Meeting';
                                    break;
                                case EventAccessType::PHYSICAL:
                                    $location = $access->address ?: 'Lokasi menyusul';
                                    break;
                                case EventAccessType::HYBRID:
                                    $location = trim(($access->address ?: 'Lokasi menyusul') . ' | Online Meeting');
                                    break;
                            }
                        } elseif ($access && $access->address) {
